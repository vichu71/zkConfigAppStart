...
public class EditDeviceVm extends SelectorComposer<Component> {
	...
	List<AddinsProp> listaPropiedadesPorIdDevice = new ArrayList<>();
	Component comp;
		...
		resClassProp.stream().forEach(prop -> {
			...
			if ("STRING_LIST".equals(prop.getType())) {
				...
				String cadena = "";
					...
					cadenaopt = listaPropiedadesPorIdDevice.stream()
...
//				valueLabelmap.put("-none-", "-none-");
	...
	private void modificarResource() throws SAXException {
		log.info("modifico-> " + addins.getAddinsDev().getName());
		...
		listaPropiedadesPorIdDevice = addins.getAddinsPropList();
...
//		} else {
			...
			Optional<String> propiedad = listaPropiedadesPorIdDevice.stream()
...
//		grid1.appendChild(columnas);
	...
	public void onSave() throws CxException {
		...
		AddinsDev addinsDev = addins.getAddinsDev();
